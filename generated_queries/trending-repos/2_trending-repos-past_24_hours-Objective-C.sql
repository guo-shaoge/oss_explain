WITH stars AS (
    SELECT
        ...
        CASE
            WHEN 'past_24_hours' = 'past_3_months' THEN SUM(
            ...
            WHEN 'past_24_hours' = 'past_month' THEN SUM(
            ...
            WHEN 'past_24_hours' = 'past_week' THEN SUM(
    ...
    WHERE
        ...
        type = 'WatchEvent'
        AND CASE
            WHEN 'past_24_hours' = 'past_3_months' THEN ge.created_at >= DATE_SUB(NOW(), INTERVAL 3 MONTH) AND ge.created_at <= NOW()
            WHEN 'past_24_hours' = 'past_month' THEN ge.created_at >= DATE_SUB(NOW(), INTERVAL 1 MONTH) AND ge.created_at <= NOW()
            WHEN 'past_24_hours' = 'past_week' THEN ge.created_at >= DATE_SUB(NOW(), INTERVAL 1 WEEK) AND ge.created_at <= NOW()
    ...
    WHERE
        ...
        AND IF('Objective-C' = 'All', TRUE, primary_language = 'Objective-C')
