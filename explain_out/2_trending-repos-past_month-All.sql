...
"    │ │   │           ├─IndexRangeScan_322(Build)","46.76","cop[tikv]","table:ge, index:index_ge_on_repo_id_created_at(repo_id, created_at)","range: decided by [eq(gharchive_dev.github_events.repo_id, gharchive_dev.github_repos.repo_id) ge(gharchive_dev.github_events.created_at, 2023-08-03 14:48:38) le(gharchive_dev.github_events.created_at, 2023-09-03 14:48:38)], keep order:false"
...
"  └─Projection_131","100.00","root","","cast(plus(plus(Column#144, ifnull(Column#188, 0)), mul(div(1, cast(plus(1, timestampdiff(YEAR, gharchive_dev.github_repos.created_at, 2023-09-03 14:48:38)), decimal(20,0) BINARY)), 200)), decimal(65,4) BINARY)->Column#193, gharchive_dev.github_repos.repo_id, gharchive_dev.github_repos.repo_name, gharchive_dev.github_repos.description, gharchive_dev.github_repos.primary_language, Column#141, Column#185"
"    └─Projection_282","100.00","root","","gharchive_dev.github_repos.repo_id, gharchive_dev.github_repos.repo_name, gharchive_dev.github_repos.description, gharchive_dev.github_repos.primary_language, gharchive_dev.github_repos.created_at, Column#141, Column#144, Column#185, Column#188"
...
"              │     └─Projection_271","217890.25","mpp[tiflash]","","Column#185, Column#186, Column#188, gharchive_dev.github_events.repo_id"
"              │       └─HashAgg_272","217890.25","mpp[tiflash]","","group by:gharchive_dev.github_events.repo_id, funcs:sum(Column#532)->Column#185, funcs:count(distinct gharchive_dev.github_events.actor_id)->Column#186, funcs:sum(Column#533)->Column#188, funcs:firstrow(gharchive_dev.github_events.repo_id)->gharchive_dev.github_events.repo_id"
...
"              │               └─Projection_281","3651465.64","mpp[tiflash]","","plus(mul(div(cast(timestampdiff(SECOND, 2023-08-03 14:48:38, gharchive_dev.github_events.created_at), decimal(20,0) BINARY), 2678400), 3), 1)->Column#538, gharchive_dev.github_events.repo_id, gharchive_dev.github_events.actor_id"
"              │                 └─Selection_258","3651465.64","mpp[tiflash]","","eq(gharchive_dev.github_events.type, ""ForkEvent""), ge(gharchive_dev.github_events.created_at, 2023-08-03 14:48:38), le(gharchive_dev.github_events.created_at, 2023-09-03 14:48:38)"
...
"                │     └─Projection_212","563651.17","mpp[tiflash]","","Column#141, Column#142, Column#144, gharchive_dev.github_events.repo_id"
"                │       └─HashAgg_213","563651.17","mpp[tiflash]","","group by:gharchive_dev.github_events.repo_id, funcs:sum(Column#523)->Column#141, funcs:count(distinct gharchive_dev.github_events.actor_id)->Column#142, funcs:sum(Column#524)->Column#144, funcs:firstrow(gharchive_dev.github_events.repo_id)->gharchive_dev.github_events.repo_id"
...
"                │               └─Projection_280","9445823.64","mpp[tiflash]","","plus(mul(div(cast(timestampdiff(SECOND, 2023-08-03 14:48:38, gharchive_dev.github_events.created_at), decimal(20,0) BINARY), 2678400), 3), 2)->Column#535, gharchive_dev.github_events.repo_id, gharchive_dev.github_events.actor_id"
"                │                 └─Selection_199","9445823.64","mpp[tiflash]","","eq(gharchive_dev.github_events.type, ""WatchEvent""), ge(gharchive_dev.github_events.created_at, 2023-08-03 14:48:38), le(gharchive_dev.github_events.created_at, 2023-09-03 14:48:38)"
"                │                   └─TableFullScan_198","6349301992.00","mpp[tiflash]","table:ge","keep order:false, PartitionTableScan:true"
"                └─Selection_167(Probe)","2533.88","root","","not(like(lower(gharchive_dev.github_repos.repo_name), ""%fuck%"", 92))"
"                  └─TableReader_166","2533.88","root","","data:Selection_165"
"                    └─Selection_165","2533.88","cop[tikv]","","eq(gharchive_dev.github_repos.is_deleted, 0), gt(gharchive_dev.github_repos.forks, 5), gt(gharchive_dev.github_repos.pushed_at, 2023-06-03 14:48:38), gt(gharchive_dev.github_repos.stars, 5), lt(gharchive_dev.github_repos.created_at, 2023-09-02 14:48:38)"
